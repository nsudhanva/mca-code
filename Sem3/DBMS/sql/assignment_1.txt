...
select branch_name, acc_no, max(amount) from deposits;

-- 7. Find all branches with assets greater than at least one branch in Brooklyn.
select branch_name, assets from banks where assets > (select count(assets) from banks where branch_city = 'Brooklyn');

-- 8. Find branches with assets greater than all branches in Brooklyn.
select branch_name, assets from banks where assets > (select sum(assets) from banks where branch_city = 'Brooklyn');
